...
入口：`python examples/blog/review-ml-foundations.py`。使用现有 NumPy、Pandas、scikit-learn、Statsmodels、Matplotlib、joblib；无网络、无新依赖、无 GPU。提取四篇可信本地文章中 18 个 `# example:` 标记代码块执行，运行前应审查源文件。不会执行错误示范或读入外部模型。

- Iris 的训练内搜索与一次最终预测，混合列缺失/未知类别，自己生成的 joblib 临时文件往返。
- 合成回归的含截距 VIF、标准化正则、数组 log、多项式特征与 RMSE/MAE。
- Wine 训练内深度选择、内置 diabetes 片段及独立合成外推边界。
- 八条聚类教学样本的候选循环不覆盖原模型，复用缩放器预测；固定双月参数作形状对比。

输出 `public/examples/blog-review-04/`：4 SVG、3 CSV、1 JSON，重跑覆盖，版本与执行范围在 JSON。搜索使用单进程；当前 Windows 环境的 joblib 多进程路径曾报 `_posixsubprocess` 缺失，未修改系统环境或声称已修复并行运行。数据是库内置或合成，不代表真实业务、线上模型部署或整篇全面复现。
