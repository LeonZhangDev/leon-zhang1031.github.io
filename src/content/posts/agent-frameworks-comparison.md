...
Agent 的核心循环可以很短，工程代码却往往不短：任务中断后从哪恢复，人工确认发生在哪一步，工具失败会不会重复写入？框架主要替你处理这些外围问题。本篇按状态管理、恢复和调试需求比较方案，不把框架流行程度当作选型依据。

下面的比较用于整理选型问题，不是统一测试环境下的性能排名。框架版本迭代较快，准备采用某项能力时，还要按目标版本核对接口与限制。
