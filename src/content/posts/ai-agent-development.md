...
“查一下机票，对比高铁，再把结论发到群里”包含几种不同动作：查询可以自动做，比较需要统一时间与费用口径，发送则需要明确授权。Agent 的作用是根据中间结果选择下一步；工程上的难点是让每一步都有边界，出错后能停下来，而不是让循环一直跑下去。

阅读时可以沿着一次任务追问：工具参数从哪里来，结果如何校验，失败后能否安全重试，哪一步必须交给人确认？这些问题决定了下面的循环能否从演示代码变成可维护的服务。
