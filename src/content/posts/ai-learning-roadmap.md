---
title: "AI 学习路线图：从 Linux 到 LLM，按前置知识选择阅读顺序"
...
文章多了之后，按发布时间阅读不一定合适：有些知识需要先学，有些可以等遇到项目问题再回查。这份路线按主题和前置关系整理已有课程。你可以从不熟悉的模块开始，已经能完成自测的部分直接跳过；周数只是安排节奏的参考，不是必须遵守的进度。

**难度标注**：⭐ 入门（零基础可读）｜⭐⭐ 进阶（需要模块内前置）｜⭐⭐⭐ 实战/深入（建议动手后再读）

![AI 学习路线图：模块依赖总览](/images/ai-learning-roadmap.png)
