...
Notebook 里看得到结果，不代表代码能从头重现结果。旧变量、跳着执行的单元格和已经存在的输出文件，都可能掩盖依赖。这里把“重启内核后顺序执行”作为交付检查，同时保留探索阶段随手试验的便利：探索可以灵活，保存的版本要说明输入与执行顺序。
...
提交前重启内核并顺序执行，检查是否依赖旧变量或旧输出。这能发现一部分隐藏状态问题，但不能替代环境、输入数据和外部服务版本的记录。
