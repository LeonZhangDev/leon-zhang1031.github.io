...
draft: false
updated: 2026-09-20
prerequisites: ["deep-learning-01-training-loop"]
verification:
  status: example-tested
  scope: "附加标量函数的 autograd 与中心差分已核对；多层网络例子未据此标为全部复现。"
...
<!-- figure:deep-learning-02-backprop -->

![把局部导数沿计算图相乘](/images/blog/deep-learning-02-backprop.svg)

*图解：与上文同一数值例子：梯度为负，因此沿负梯度更新会增加 w 和 b。*

这个例子的输入固定，更新 w 与 b 都会改变同一个预测。一次恰好命中标签来自数值和学习率的特殊组合，不是梯度下降的普遍保证。如果把学习率改为 1，预测会跨过目标；如果损失写成半平方误差，梯度数值也会减半。比较实现前必须先比较损失定义。

**动手核对：** 令有限差分步长为 1e-5，分别只扰动 w、只扰动 b，比较中心差分与解析梯度。验收关注绝对误差和浮点类型，而不只打印“通过”。
...
有限差分适合检查小模型的梯度实现，而不是替代训练中的自动微分。步长过大会引入近似误差，过小又会受浮点精度影响；在 ReLU 的拐点附近，两边差分与某个自动微分约定不同也不一定说明实现错误。
...
附加实验取 `f(x)=(x²-2)²`，在 `x=0.7`、float64 下求导。PyTorch autograd 得到 `-4.228`；步长 `1e-5` 的中心差分得到 `-4.2279999997063555`，绝对差小于 `1e-7`。完整函数 `gradient_check()` 位于仓库 [`examples/blog/run-examples.py`](https://github.com/LeonZhangDev/leon-zhang1031.github.io/blob/main/examples/blog/run-examples.py)，[本次结果和环境](/examples/blog/results.json)可下载核对。

这里选择平滑函数是为了隔离自动微分链路。若点落在 ReLU 的零点附近，中心差分可能跨过不可导点；若步长过小，则可能受到浮点消减误差影响。两种情况都不应直接归因为框架算错。数值差分计算成本较高，适合小规模检查而不是替代训练中的反向传播。
