...
<!-- figure:deep-learning-03-training-stability -->

![先定位错误所在层](/images/blog/deep-learning-03-training-stability.svg)

*图解：按依赖顺序排查：数据或更新链路失败时，复杂调参很难提供有效信息。*

每次只改变一个诊断条件，保留相同的数据划分与随机种子。loss 出现 NaN 时先记录第一个非有限值的位置，不要先通过截断 loss 掩盖问题。小样本能拟合后，再看训练与验证的差距；两者都差与训练好验证差，需要不同的下一步。

**动手核对：** 保存正常、学习率过大、标签打乱三份实验配置。由日志绘图，并在图注写清样本数、步数、随机种子，不能将理论示意曲线当作运行结果。
...
先确认两条曲线的统计口径一致：训练损失是否含正则项，是否启用增强，记录的是最后一个 batch 还是整轮按样本加权平均？口径不同造成的差距，不应立刻解释成过拟合。把这些条件写在图旁边，后面的诊断才有依据。
