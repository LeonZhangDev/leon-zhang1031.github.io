...
文中的具体表格属于**参考运行记录**，用于展示怎样分析结果，不代表作者亲自在某台设备上完成了对应运行。
...
<!-- figure:deep-learning-05-transfer-learning-project -->

![分阶段迁移与选择](/images/blog/deep-learning-05-transfer-learning-project.svg)

*图解：验证集用于选择解冻范围和训练轮次；测试集用于最终报告。*

如果同一对象的近重复照片同时进入训练和测试，即使使用预训练模型也会得到失真的泛化估计。冻结参数还要检查 BatchNorm 统计量是否更新：requires_grad=False 与 eval() 不是同一件事。保存模型时同时保存类别到编号的映射和预处理参数。

**动手核对：** 比较分类头训练与部分解冻时，保持数据划分、增强和评价指标一致；报告失败类别，而不是只选总体准确率最高的一次。

## 5. 分阶段解冻，不要一把梭哈

解冻时要确认新解冻的参数已加入优化器；只改 `requires_grad`，若优化器最初只接收分类头参数，骨干网络仍不会更新。BatchNorm 的运行统计又是另一层状态，需要明确冻结特征时是否同时固定这些统计量。
