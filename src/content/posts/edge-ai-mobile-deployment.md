...
下面以拍照分类串起导出、转换、量化和移动端调用。多次格式转换会增加兼容风险；如果目标运行时能直接支持训练框架的导出格式，应优先比较更短的路径，而不是机械地经过每一个中间格式。
...
仅凭模型名称无法给出手机端延迟。没有配套设备、模型文件与运行日志时，不应填入看似精确的实测值。可以先按下面的比较维度准备验证：

| 对照 | 需要记录 | 结果怎样解释 |
| --- | --- | --- |
| 浮点与 INT8 | 文件体积、任务指标、端到端延迟 | 体积减少不代表延迟同比减少 |
| CPU 与硬件委托 | 支持算子、设备切换、预热后延迟 | 部分回退可能抵消加速收益 |
| 单次与持续推理 | 温度、功耗、长时间延迟变化 | 冷机的快结果不能代表持续使用 |

Android 与 iOS 的模型格式和后端不同，应分别记录部署产物，不能把同一行精度直接套用到两个平台。

## 云端协同：不是二选一，是分层

把请求送往云端之前，需要定义离线行为：继续使用端侧低精度结果、提示稍后重试，还是暂停功能？回退策略不能悄悄上传原本承诺留在设备上的数据。端云协同首先是产品与数据边界的约定，其次才是推理引擎选择。
