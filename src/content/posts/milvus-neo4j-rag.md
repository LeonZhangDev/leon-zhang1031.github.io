---
title: "Milvus + Neo4j 构建 RAG 知识图谱实战"
date: 2025-08-10T00:00:00+08:00
draft: false
author: "Zack-Zhang1031"
description: "梳理 RAG / GraphRAG / HyDe / Reranker 的核心原理与适用场景，对比向量数据库 Milvus 与图数据库 Neo4j 的分工，并给出 pymilvus 检索示例代码。"
tags: ["AI", "RAG", "Milvus", "Neo4j", "向量数据库", "知识图谱"]
categories: ["AI", "检索增强生成"]
---

做 AI 应用绕不开「检索增强生成（RAG）」这条技术路线，而要让检索真正落地，向量数据库和图数据库各有所长。这篇笔记先把 Milvus 与 Neo4j 的定位讲清楚，再梳理 RAG、GraphRAG、HyDe、Reranker 这几个高频概念的关系与适用场景，作为后续动手搭一套知识图谱问答系统的基础。
...
图数据库与向量数据库解决不同查询需求，不必为了使用 GraphRAG 就同时引入两套存储。先列真实问题：需要关系遍历才保存图结构，需要语义近邻才建立向量索引；两种结果最终都应能回到原始证据，而不是只返回一个看似相关的节点。
