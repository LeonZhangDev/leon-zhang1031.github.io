...
OpenCV 图像处理的入门必修课：插值方法怎么选、掩膜怎么做、ROI 怎么切、水印怎么贴、灰度化的三种公式有什么区别。这篇笔记把原理和代码对照着讲，并在每个小节末尾标出"常见误区"，适合作为速查手册反复回看。
...
图像与掩膜的缩放方式要分别选择。照片可以用连续插值，类别标签掩膜通常应避免生成不存在的中间类别值。检查 shape 之外，还要检查 dtype、通道数和掩膜取值范围；尺寸一致并不代表语义一致。
