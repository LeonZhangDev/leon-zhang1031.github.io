...
模型把 JSON 写成了 Markdown，或把信息提取变成了自由续写，这类问题很适合用来理解提示设计。提示需要说清任务、输入边界和输出格式，也需要和校验代码、测试样本配合。下面从这些具体失败开始，逐步改进提示与调用流程。
...
`judge_fn` 可以分两类：有标准答案的分类、提取任务，先归一化再比对；摘要、写作等开放任务，可以让模型按明确量表辅助评分。后一种方式要用人工标注样本校准，并检查位置偏好、表达风格偏好和重复评分波动。一致性与成本都应在自己的任务上测量，不能由模型名称推断。
