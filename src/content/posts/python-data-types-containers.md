...
description: "系统梳理 Python 五大容器类型（list/tuple/range/set/dict）的创建方式、常用操作、可变性与哈希性对比，并解读 dict 赋值即存储的语义，附一图流对比表。"
tags: ["Python", "数据结构", "容器类型"]
...
元组不可变，指的是它保存的元素引用不能被替换，并不保证元素本身不可变。例如元组里放一个列表，列表仍可追加元素。这也解释了为什么不是每个元组都能作为字典键：还要看里面的元素是否可哈希。
