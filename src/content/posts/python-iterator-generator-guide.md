...
description: "用超市买零食的生活类比讲透 Python 迭代器与生成器：协议实现、yield 惰性求值、send 协程、itertools 实战，附斐波那契生成器示例。"
tags: ["Python", "迭代器", "生成器", "惰性求值"]
...
同一个生成器实例通常只消费一次。若日志预览先把它遍历完，再交给统计函数，后者可能收到空数据。需要重复读取时，应重新创建生成器，或在确认内存可接受后物化；这不是给对象再套一个 `iter()` 就能重置的。
