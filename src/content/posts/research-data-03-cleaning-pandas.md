...
前两课定义了采集流程，这一课把不同来源的记录整理成能共同使用的数据表。关键不只是去掉空值，还要解释一行代表什么、同一篇论文怎样合并、冲突字段保留哪一份。只有这些规则明确，后面的统计和建模才有稳定的输入。
...
审计时应按来源分别查看缺失和格式差异。例如预印本可能尚无正式发表 venue，网页里的年份也可能写成“in press”。这些情况需要不同处理：尚未发生的发表信息不能凭空补齐，无法解析的年份应保留原值并标记。先处理影响主键、切分和标签的字段，再决定其他字段是否保留；缺失率高本身并不是删除字段的充分理由。
