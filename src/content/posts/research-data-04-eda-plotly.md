...
清洗后字段能用，并不代表已经理解数据。这一课用分布、时间趋势和字段关系检查数据集：引用量是否长尾，某年的记录为何减少，缺失是否集中于某个来源？每张图都要对应一个问题，异常则回到原始记录核对，避免给采集缺口编出业务故事。
...
先观察当前语料是否呈右偏长尾，不要在画图前就写结论。论文年份、领域与采集筛选都会改变分布。若少量大值占据较大范围，可以进一步考虑：
...
最近年份可能尚未收全，当前年份也没有走完。若曲线末端下降，先排查观察窗口与采集滞后，再讨论领域变化；可排除未完整年份，或明确标注其覆盖范围。不能看到下降就断定是采集问题或领域遇冷。
