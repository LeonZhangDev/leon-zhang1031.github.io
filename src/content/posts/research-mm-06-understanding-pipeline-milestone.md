...
这是展示控制流的伪代码，`load_json`、`save_json` 和五个阶段都需另行实现；跳过 `done` 不是幂等性的充分条件。状态应至少包含论文 ID、输入摘要、阶段实现版本、产物校验和。解析器升级后，S1 及依赖其结果的下游阶段都要失效；只清 S1 会混用新解析和旧向量。产物应先写临时文件、校验完成后原子替换，再提交状态。进程在二者之间中断时，由恢复逻辑检查产物，而不是仅相信状态字符串。
...
标题自检是低成本冒烟线索，不应保证一定排第一：同名论文、重复版本和近似检索都会影响排名。先验证 ID—向量映射，再在规定 K 内检查目标是否存在，并单独测试真实查询的相关性。此处 `all([])` 会返回真，图表列表为空不代表抽取成功；需要结合论文是否有图、阶段成功状态及抽样标注判断。`sections >= 3` 也只是教学阈值，不能直接代表解析完整。
