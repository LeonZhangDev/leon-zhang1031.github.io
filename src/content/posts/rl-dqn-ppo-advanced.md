...
表格 Q-Learning 需要为状态和动作保存数值，面对图像输入时难以直接扩展。用神经网络近似 Q 函数后，又会遇到相关样本和不断变化的目标。本文从这些训练困难解释 DQN 的经验回放与目标网络，再比较 PPO 的策略更新思路，不把某一种算法当作所有任务的默认答案。
...
保留完整 DQN 作为基线，分别修改经验回放和目标网络，比较多个随机种子的回报曲线、Q 值范围和失败比例。这些组件旨在缓解训练不稳定，但去掉组件并不保证每次都会崩溃，单个种子的曲线也不能证明普遍结论。

PPO 的 `clip_range` 同样应在固定采样预算下比较，而不是预设某个值必然发散。这里给出实验设计，不提供未运行的回报数字。记录环境版本、终止条件和评估策略，才能解释配置变化带来的影响。
