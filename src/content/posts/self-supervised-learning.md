...
无标注数据可以怎样参与训练？自监督学习从数据本身构造预测任务，例如遮住一部分文本再预测，或让同一张图片的不同增强视图靠近。这里先讨论监督信号从哪里来，再比较不同目标保留了什么信息，以及如何评估学到的表示。
...
代理任务能鼓励模型保留某些输入结构，但不能保证学到下游任务的充分统计量。预训练数据、增强方式和任务目标与下游是否匹配，都影响迁移结果。预训练与适配可以按下面的流程理解：
...
```python
# 实验设计：CIFAR-10 有 10 类，只保留部分训练标签
# 方案 A：直接在 500 张标注图上从头训练 ResNet18
# 方案 B：先在 50000 张无标注图上 SimCLR 预训练，再只用 500 张微调分类头

# 方案 C：使用全部训练标签监督学习，作为另一项对照
# 记录相同测试集上的准确率、多种子差异与训练成本；此处不预填结果
```

这个设计检查无标注数据是否帮助下游任务，并不保证方案 B 接近全监督结果。无标注数据也有采集、清洗和预训练成本；测试集不能进入无标注预训练语料，除非明确研究的是允许这种访问的设定。
...
选择自监督方案时，回到三个问题：预训练任务保留了什么信息，这些信息是否适合下游任务，以及收益是否覆盖计算成本。无标注数据提供了更多训练材料，但数据质量、分布差异和评估方法仍然需要认真处理。
