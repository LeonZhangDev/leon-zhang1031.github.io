...
description: "从 JOIN、窗口函数到索引与执行计划，结合 Python 连接与数据建模，梳理 AI 项目中常见的数据库查询与排错问题。"
tags: ["SQL", "MySQL", "数据库", "数据分析", "窗口函数"]
categories: ["AI课程", "数据工程"]
math: false
---

训练数据提取、特征回写和报表统计都离不开 SQL。查询能运行只是第一步，还需要确认连接有没有放大行数、聚合有没有改变口径、索引是否适合实际访问方式。下面用这些问题串起基础查询和数据库实践。
