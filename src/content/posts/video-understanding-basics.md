...
视频任务需要先处理帧率、时间戳、存储和解码，再谈模型。采样间隔不同，得到的动作片段和计算成本都会变化。这篇先明确时间维度带来的约束，再介绍动作识别和视频特征提取，帮助你把输入处理与评估口径对齐。

先检查抽帧加图像模型是否足以回答任务，再讨论需要顺序和运动信息的 3D CNN、Video Transformer。静态物体存在性与动作方向不是同一种问题，抽帧基线的价值在于帮助确定是否确实需要时序建模。
...
如果目标在单帧内即可判断，可以先用抽帧分类建立基线，但必须检查短暂事件会不会落在采样间隔里。若任务区分开门与关门，单帧往往不够；增加抽帧数量也不等于模型已经利用顺序。
